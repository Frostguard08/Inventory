...
if ($_SERVER['REQUEST_METHOD'] === 'POST') { 
    ...
    foreach ($row as $i => $products) {
        ...
        array_push($itemArr,  array($products['item_id'], $products['quantity']));
    }

    $cash = $_POST['cash'];
    if ($cash < $total_amount) {
        echo "<script>alert('Insufficient Cash!'); window.location = 'order_process.php'; </script>";
        exit();
    }
    ...
    if (empty($errors)) {
        $statement = $pdo->prepare("INSERT INTO tbl_orders (order_date, total_quantity, total_amount, inventory_ids, serial_number, cash)
          VALUES (:order_date, :total_quantity, :total_amount, :inventory_ids, :serial_number, :cash)"
        );
